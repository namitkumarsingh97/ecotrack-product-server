...
export const generatePDFReport = async (companyId: string, period?: string): Promise<Buffer> => {
  ...
  const [envMetrics, socialMetrics, govMetrics] = await Promise.all([
    EnvironmentalMetrics.findOne({ companyId, period: esgScore?.period }).sort({ createdAt: -1 }),
    SocialMetrics.findOne({ companyId, period: esgScore?.period }).sort({ createdAt: -1 }),
    GovernanceMetrics.findOne({ companyId, period: esgScore?.period }).sort({ createdAt: -1 })
  ]);
  ...
  return new Promise((resolve, reject) => {
    ...
    // Footer
    doc.fontSize(9).text(
      `Generated by EcoTrack India on ${new Date().toLocaleDateString('en-IN')}`,
      { align: 'center' }
    );
